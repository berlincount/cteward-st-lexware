...
function realstatus(member) {
    ...
    if (member.Austritt && member.Austritt !== '') {
        ...
        var austritt = new Date(member.Austritt);
...
function datum(isodate) {
    ...
    var date = strptime(isodate, '%Y%m%d');
...
function patenarray(patenstr) {
    ...
    if (patenstr.indexOf(',') < 0 )
        return [ patenstr.trim() ];

    return patenstr.split(',').map(trim).filter(function(element) { return element !== ''; });
}
