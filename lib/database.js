          ...
          for (var i=0; i < data.length; i++) {
            ...
            for (var j=0; j < data[i].length; j++) {
              ...
              if (data[i][j].Geburtsjahr) {
                startjahr = data[i][j].Geburtsjahr - (data[i][j].Geburtsjahr % 5);
                endjahr   = startjahr + 5;

                if (startjahr < minjahr)
                  minjahr = startjahr;
                if (endjahr   > maxjahr)
                  maxjahr = endjahr;
                range     = startjahr+"-"+endjahr;
              } else
                range = "Other";

              if (range in entry)
                entry[range] += data[i][j].Anzahl;
              else
                entry[range]  = data[i][j].Anzahl;
            }

            console.log("minjahr:"+minjahr+" maxjahr:"+maxjahr);
            for (var jahr = minjahr; jahr < maxjahr; jahr += 5) {
              range     = jahr+"-"+(jahr+5);
