...
module.exports = {
  ...
  runquery: Promise.method(function runquery(v) {
    ...
    return request.queryAsync(v.query.statement).then(function(dbresponse) {
  ...
  runquery_stats_members: Promise.method(function runquery_stats_members(v) {
    ...
    return request.queryAsync("SELECT MIN(Eintritt) AS EintrittMin,MAX(Eintritt) AS EintrittMax,MIN(Austritt) AS AustrittMin,MAX(Austritt) AS AustrittMax FROM Adresse").then(function(dbresponse) {
  ...
  runquery_stats_contracts: Promise.method(function runquery_stats_contracts(v) {
    ...
    return request.queryAsync("SELECT MIN(VertragBegin) AS VertragBeginMin,MAX(VertragBegin) AS VertragBeginMax,MIN(VertragEnde) AS VertragEndeMin,MAX(VertragEnde) AS VertragEndeMax FROM MgVert").then(function(dbresponse) {
      var mindate = Date.now();
      var maxdate = Date.now();
      data = dbresponse.recordset;
      for (var key in data[0]) {
        ...
        return Promise.all(queries).then(function(data) {
          ...
          for (var i=0; i < data.length; i++) {
            ...
            for (var j=0; j < data[i].length; j++) {
              ...
              if (data[i][j].Firmenname !== null && data[i][j].Firmenname !== '' && data[i][j].Firmenname.indexOf('c/o ') === -1) {
                entry.Business+=data[i][j].Anreden;
                //console.log("Business += "+data[i][j].Anreden);
              } else if (data[i][j].Geschlecht == 'MÄNNLICH') {
                entry.Male+=data[i][j].Geschlechter;
                //console.log("Male += "+data[i][j].Geschlechter);
              } else if (data[i][j].Geschlecht == 'WEIBLICH') {
                entry.Female+=data[i][j].Geschlechter;
                //console.log("Female += "+data[i][j].Geschlechter);
              } else if (data[i][j].Anrede == 'Herr') {
                entry.Male+=data[i][j].Anreden;
                //console.log("Male += "+data[i][j].Anreden);
              } else if (data[i][j].Anrede == 'Frau') {
                entry.Female+=data[i][j].Anreden;
                //console.log("Female += "+data[i][j].Anreden);
              } else {
                entry.Other+=data[i][j].Anreden;
                //console.log("Other += "+data[i][j].Anreden);
              }
            }
            result.push(entry);
          }
  ...
  memberlookup: Promise.method(function memberlookup(crewname, v) {
    ...
    return request.queryAsync('SELECT Kurzname,Kennung3,Eintritt,Austritt FROM Adresse WHERE Kurzname = @crewname').then(function(dbresponse) {
      data = dbresponse.recordset;
      if (data.length != 1)
