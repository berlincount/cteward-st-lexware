...
module.exports = function () {
  ...
  server.use(restify_plugins.authorizationParser());
  server.use(restify_plugins.gzipResponse());
  server.use(restify_plugins.requestLogger());
  server.use(restify_plugins.queryParser());
  server.use(restify_plugins.fullResponse());
  ...
  server.pre(function (request, response, next) {
     ...
     if (request.headers.authorization) {
         ...
         if (auth[0] == 'Basic') {
           var authdata = Base64.decode(auth[1]);
           username = authdata.toString().split(':')[0];
         }
     } else {
  ...
  server.get('/legacy/stats/members', function handle_legacy_stats_members(req, res, next) {
    ...
    authprovider.authorize({
        ...
        permissions: {
          ...
          '_stats_':     { query: database.QUERY_STATS_MEMBERS, level: 0 },
        },
      })
      .then(database.runquery)
      .then(filters.runfilter)
  ...
  server.get('/legacy/stats/contracts', function handle_legacy_stats_contracts(req, res, next) {
    ...
    //  FIXME: example
    // Permissions:
    //  board     : can see full historic data
    authprovider.authorize({
        config:      config,
        request:     req,
        response:    res,
        permissions: {
          ...
          '_stats_':     { query: database.QUERY_STATS_CONTRACTS, level: 0 },
        },
      })
      .then(database.runquery)
      .then(filters.runfilter)
  ...
  server.get('/legacy/stats/genders', function handle_legacy_stats_genders(req, res, next) {
    // Parameters:
    //  None
    // Returns JSON:
    //  FIXME: example
    ...
    authprovider.authorize({
        ...
        response:    res,
        permissions: {
          '_bot_':       { query: database.QUERY_STATS_GENDERS, level: 0 },
          '_board_':     { query: database.QUERY_STATS_GENDERS, level: 0 },
          '_stats_':     { query: database.QUERY_STATS_GENDERS, level: 0 },
        },
      })
      .then(database.runquery)
      .then(filters.runfilter)
  ...
  server.get('/legacy/stats/ages', function handle_legacy_stats_ages(req, res, next) {
    // Parameters:
    //  None
    // Returns JSON:
    //  FIXME: example
    ...
    authprovider.authorize({
        ...
        response:    res,
        permissions: {
          '_bot_':       { query: database.QUERY_STATS_AGES, level: 0 },
          '_board_':     { query: database.QUERY_STATS_AGES, level: 0 },
          '_stats_':     { query: database.QUERY_STATS_AGES, level: 0 },
        },
      })
      .then(database.runquery)
      .then(filters.runfilter)
